...
function getGovernanceNomData() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Nom');
...
function updateGovernanceNomData(updatedData) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Nom');
  var data = sheet.getDataRange().getValues();
...
function addGovernanceNomData(newData) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Nom');
...
function getNomPercentage() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza');
  var percentage = sheet.getRange('D3').getValue(); // Make sure this is the correct cell for the percentage
  return percentage;
}
...
function deleteGovernanceNomData(id) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Nom');
  var data = sheet.getDataRange().getValues();
...
function getGovernanceNomDataForClient() {
  return getGovernanceNomData();
}


// Function to handle updating data from client side
function handleUpdateNom(updatedData) {
  updateGovernanceNomData(updatedData);
}

// Function to handle adding new data from client side
function handleAddNom(newData) {
  addGovernanceNomData(newData);
}

// Function to handle deleting data from client side
function handleDeleteNom(id) {
  deleteGovernanceNomData(id);
}


function getNomPercentageForClient() {
  return getNomPercentage();
}
