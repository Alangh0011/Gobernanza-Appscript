...
function getGovernanceTecnicoData() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Técnico');
...
function updateGovernanceTecnicoData(updatedData) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Técnico');
  var data = sheet.getDataRange().getValues();
...
function addGovernanceTecnicoData(newData) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Técnico');
...
function getTecnicoPercentage() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza');
  var percentage = sheet.getRange('D5').getValue(); // Make sure this is the correct cell for the percentage
  return percentage;
}
...
function deleteGovernanceTecnicoData(id) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Gobernanza - Técnico');
  var data = sheet.getDataRange().getValues();
...
function getGovernanceTecnicoDataForClient() {
  return getGovernanceTecnicoData();
}


// Function to handle updating data from client side
function handleUpdateTecnico(updatedData) {
  updateGovernanceTecnicoData(updatedData);
}

// Function to handle adding new data from client side
function handleAddTecnico(newData) {
  addGovernanceTecnicoData(newData);
}

// Function to handle deleting data from client side
function handleDeleteTecnico(id) {
  deleteGovernanceTecnicoData(id);
}


function getTecnicoPercentageForClient() {
  return getTecnicoPercentage();
}
